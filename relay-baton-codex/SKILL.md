...
Cache writes exist, but their processing is covered by full-rate noncached input in Codex credit billing; reused cached input is 0.1x. The renderer shows `cache-write unmeasured*` because the native reported count does not establish actual cache creation volume, while preserving that raw count in caveats and JSON. `in(noncache)` is total input minus cache reads and already includes any cache creation, so do not add the write count again or apply a separate surcharge. Not every noncached input token necessarily becomes cached. The Standard API benchmark uses direct API price semantics for reported categories, may omit unreported cache creation, and is neither Codex credit usage nor billed spend. A published API cache-retention minimum is not a measured TTL for a Codex task, so task TTL remains unavailable unless directly observed.
