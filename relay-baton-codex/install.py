...
Render cache writes as `unmeasured*`. Preserve the raw native count while stating that actual creation volume is not established. Codex credit billing covers any cache creation inside full-rate noncache=input minus cache-read and discounts reused cache-read to 0.1x; do not add the write field again or call it free processing. Direct API benchmark semantics remain separate from Codex credits and billed spend.
